...
namespace WindowsPractice.Ui.Tests
{
    public class Robot
    {
        ...
        //test
        public void MouseDownHoldMoveReleaseTheControl(string name, int x, int y, int dx, int dy)
        {
        ...
        // test
        public void ZoomShape(int dx, int dy)
        {
            Actions action = new Actions(_driver);
            action
                .ClickAndHold()
        ...
        // test
        public void MoveShape(int smallDx, int smallDy, int dx, int dy)
        {
            Actions action = new Actions(_driver);
            action
                .MoveByOffset(-smallDx, -smallDy)
                .ClickAndHold()
                .MoveByOffset(dx, dy)
                .Release()
                .Perform();
        ...
        public void ClickDataGridViewDelete(string name, int row)
        {
            Actions action = new Actions(_driver);
            WindowsElement element = _driver.FindElementByAccessibilityId(name);
            var delete = element.FindElementByName($"刪除 資料列 {row}");
            action
                .Click(delete)
                .Perform();
        }
        ...
        //test
        public void AssertRatio(string name, float ratio)
        {
            WindowsElement element = _driver.FindElementByAccessibilityId(name);

            float controlRatio = (float) element.Size.Width / element.Size.Height;
            Assert.IsTrue(Math.Abs(controlRatio - ratio) < 0.08f);
        }
        ...
        //test
        public void AssertToolStritButtonChecked(string name)
        {
            WindowsElement element = _driver.FindElementByName(name);
            const string CHECK_STATE = "1048724";
            Assert.AreEqual(CHECK_STATE, element.GetAttribute("LegacyState"));
        }

        //test
        public void AssertDataGridViewShapeAndLocationCell(string name, int row, string shapeName, string location)
        {
            WindowsElement element = _driver.FindElementByAccessibilityId(name);
            var rowShapeData = element.FindElementByName($"形狀 資料列 {row}");
            var rowLocationData = element.FindElementByName($"資訊 資料列 {row}");

            Assert.AreEqual(shapeName, rowShapeData.GetAttribute("Value.Value"));
            Assert.AreEqual(location, rowLocationData.GetAttribute("Value.Value"));
        }

        //test
        public void AssertDataGridViewCount(string name, string count)
        {
            WindowsElement element = _driver.FindElementByAccessibilityId(name);

            Assert.AreEqual(count, element.GetAttribute("Grid.RowCount"));
        }
